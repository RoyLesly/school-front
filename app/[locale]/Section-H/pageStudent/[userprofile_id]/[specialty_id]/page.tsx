...
const page = async ({
  ...
  const profile: any = await getData(GetUserProfileUrl, { id: params.userprofile_id, fieldList: [
  ...
  const apiNotiSchool: any = await getData(GetNotificationUrl, { target: "school", schools: profile.results[0].specialty__school__campus__id, status: true, fieldList: [
    "id", "message_one", "message_two", "noti_type",
  ] });
  const apiNotiDomain: any = await getData(GetNotificationUrl, { target: "domain", domains: profile.results[0].specialty__main_specialty__field__domain__id, status: true, fieldList: [
    "id", "message_one", "message_two", "noti_type",
  ] });
  const apiNotiSpecialty: any = await getData(GetNotificationUrl, { target: "specialty", specialty: profile.results[0].specialty__id, status: true, fieldList: [
    "id", "message_one", "message_two", "noti_type",
  ] });

  console.log(profile)
  console.log(apiNotiSchool)
  console.log(apiNotiDomain)
  console.log(apiNotiSpecialty)
      ...
      {profile && profile.count && <section className="my-6 px-3">
        ...
        <div className='gap-3 grid grid-cols-2 grid-rows-3 mt-3 p-3 pb-16 rounded-[16px]'>
          {menuList.map((list: any, index: number) => {
            return (
              <Link href={`/pageStudent/${params.userprofile_id}/${params.specialty_id}/${list.label}`} key={list.id} className='bg-white flex flex-col h-[86px] items-center justify-center rounded-[20px] shadow-3xl w-full'>
