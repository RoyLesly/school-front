...
import { GetDomainInter, GetLevelInter, GetMainSpecialtyInter, GetSpecialtyInter } from '@/Domain/Utils-H/appControl/appInter';
...
const SpecialtyAssignment: FC<SpecialtyAssignmentProps> = ({ domain, mainSpecialty, academicYear, level, onSubmitServerAction, school_id }) => {
    ...
    useEffect(() => {
        ...
        if (count == 1 && selDomainID && selAcademicYear.length > 4 && selLevelID) {
            const GetSpecialties = async () => {
                var res = await getData(protocol + "api" + dom + GetSpecialtyUrl, { main_specialty__field__domain__id: selDomainID, level__id: selLevelID, academic_year: selAcademicYear, school__campus__id: school_id, fieldList: [ 
    ...
    }, [count, dom, selDomainID, loading, domain, selAcademicYear, mainSpecialty, level, selLevelID, academicYear, school_id])
