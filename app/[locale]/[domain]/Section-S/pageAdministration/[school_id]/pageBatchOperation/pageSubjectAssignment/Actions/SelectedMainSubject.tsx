    ...
    const submit = async () => {
        ...
        if (checkCode.length < 1 && checkCredit.length < 1 && checkType.length < 1 && pathname) {
            ...
            if (data && data.length > 0 && pn) {
                ...
                .catch(err => {
                    if (pn){
                        router.push(`/Section-H/pageAdministration/${pn[0]}/pageBatchOperation/pageSubjectAssignment?classroom_id=${searchParams.get("classroom_id")}&error=${JSON.stringify(err)}`);
