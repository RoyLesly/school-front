...
const MarksForm = ({ resultData, params }: any) => {
    ...
    const FieldChanged = (e: any, result: GetResultInter) => {
        ...
        if (e.value > ConfigData.exam_limit) { return }
        ...
        var newRes: SelectedResultInter = {
            ...
            exam: e.name == "exam" ? e.value : result.ca,
            ...
            await Promise.allSettled(ArrayMarksToSubmit.map((item: SelectedResultInter, index: number) => {
                ...
                .then(res => {
                    ...
                    if (res && res.length > 0) {
                        ...
                        if (t[0] == "fulfilled") { router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry?updated=SUCCESSFULLY !!!`); }
                        else { router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry?error=${t[0]}`); }
                    }
                })
                .catch(err => {
                    console.log(98, err)
