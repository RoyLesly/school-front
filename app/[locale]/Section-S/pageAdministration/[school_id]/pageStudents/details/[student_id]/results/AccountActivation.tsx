...
const AccountActivation = async ({ params }: any) => {
  ...
  return (
    <div className="flex flex-col flex-grow gap-10 h-full items-center justify-center py-40 text-black text-xl">
        ...
        <Link href={`/Section-S/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation`} className='bg-bluedash px-6 py-2 rounded text-white'>{t("pay_now")}</Link>
    </div>
  )
}
