...
import { CustomUserUrl, DepartmentUrl, GetUserProfileUrl } from '@/Domain/Utils-H/userControl/userConfig';
import { DeptInter, GetUserProfileInter } from '@/Domain/Utils-H/userControl/userInter';
...
const page = async ({
  ...
  params: { id: string, school_id: string, domain: string  };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData: GetUserProfileInter | any = await getData(protocol + "api" + params.domain + GetUserProfileUrl, { id: params.id, nopage: true });
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, id, params }) => {
  ...
  const deptData: DeptInter | any = await getData(protocol + "api" + params.domain + DepartmentUrl, {});
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    console.log(apiData)
    // return
    const response = await ActionEdit(data, apiData.user__id, SchemaEditCustomUser, protocol + "api" + params.domain + CustomUserUrl)
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(protocol + "api" + params.domain + UserProfileUrl, apiData.user__id)
