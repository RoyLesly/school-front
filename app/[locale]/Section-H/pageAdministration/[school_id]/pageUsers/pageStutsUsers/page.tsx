...
const page = async ({
  ...
  const apiData: CustomUserInter[] | any = await getData(protocol + CustomUserUrl, {...searchParams, role: "student", school__id: params.school_id, is_superuser: false});
  ...
  return (
    <LayoutAdmin>
      ...
      <Breadcrumb
          ...
          link2="/Section-H/pageAdministration/pageUsers" 
        />
  ...
  return (
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      ...
      <MyPagination
        ...
        count={apiData.count}
        thisUrl={`/Section-H/pageAdministration/${school_id}/pageUsers/pageStutsUsers`}
      />
...
const SearchUser = ({ school_id }: any) => {
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (full_name) {
        redirect(`/Section-H/pageAdministration/${school_id}/pageUsers/pageStutsUsers?full_name=${full_name}`)
    }
  ...
  return (
      <>
          ...
          <form action={onSearchDrugServerAction} className='flex flex-row gap-1'>
              ...
              <Link className='flex items-center justify-center' href={`/Section-H/pageAdministration/${school_id}/pageUsers/pageStutsUsers`}><FiRefreshCw size={23} /></Link>
          </form>
