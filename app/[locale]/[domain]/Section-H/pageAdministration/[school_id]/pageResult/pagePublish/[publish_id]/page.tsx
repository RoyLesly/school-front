...
const page = async ({
  ...
  params: { school_id: string,  domain: string, publish_id: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData = await getData(protocol + "api" + params.domain + GetPublishUrl, { id: params.publish_id, fieldList: [
...
const PublishConfirm = ({ apiData, params, searchParams }: any) => {
  ...
  const onSubmitSearchAction = async () => {
    ...
    if (response.id == apiData.id){
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pagePublish?success=Published Successfully`)
    }
