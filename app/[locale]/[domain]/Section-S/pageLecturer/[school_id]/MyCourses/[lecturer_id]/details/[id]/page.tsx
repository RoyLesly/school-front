...
const EditPage = async ({
  ...
  const apiData: any = await getData(protocol + GetCourseUrl, { id: params.id, fieldList: [ 
    "id", 
...
const EditDelete:FC<EditDeleteProps> = ( { apiData, params }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.level_id, SchemaCreateEditLevel, protocol + CourseUrl, `/Section-S/pageLecturer/${params.school_id}/pageSettings/pageLevels`)
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(CourseUrl, params.id)
