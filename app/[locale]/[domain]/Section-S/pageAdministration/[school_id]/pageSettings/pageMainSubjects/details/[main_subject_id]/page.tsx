...
import { GetMainSubjectInter } from '@/Domain/Utils-S/appControl/appInter';
...
import { ActionDelete } from '@/serverActions/actionGeneral';
...
const EditPage = async ({
    ...
    params: { school_id: string, main_subject_id: string, domain: string };
    searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData: GetMainSubjectInter | any = await getData(protocol  + "api" + params.domain + GetMainSubjectUrl, { id: params.main_subject_id, nopage: true });
...
const Edit:FC<EditProps> = async ( { apiData, params }) => {
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(MainSubjectUrl, params.main_subject_id)
