...
import { GetSecondaryProfileUrl, SecondaryProfileUrl } from '@/Domain/Utils-S/userControl/userConfig';
...
import { SchemaAssignClassroom } from '@/Domain/Utils-S/schemas/schemas';
...
const AssignSpecialty: FC<AssignSpecialtyProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.student_id, SchemaAssignClassroom, SecondaryProfileUrl, `/pageAdministration/${params.school_id}/pageStudentsUnAssigned`)

    if (response?.detail) {
      redirect(`/pageAdministration/${params.school_id}/pageStudentsUnAssigned/${params.student_id}?error=Error-${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/pageAdministration/${params.school_id}/pageStudents?updated=${response.user.full_name} - Assigned Successfully`)
    } else {
      redirect(`/pageAdministration/${params.school_id}/pageStudentsUnAssigned/${params.student_id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
