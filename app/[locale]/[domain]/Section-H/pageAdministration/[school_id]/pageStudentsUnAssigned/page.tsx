...
const StudentList = async ({
  ...
  const apiData: any[] | any = await getData( protocol + "api" + params.domain + GetUserProfileUrl, {...searchParams, user__role: "student", nospecialty: true, user__school__id: params.school_id, nopage: true, fieldList: [       
  ...
  return (
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex flex-col gap-2 justify-between md:flex-row md:px-6 px-4 py-6 xl:px-7.5">
        ...
        <SearchUserProfile school_id={params.school_id} params={params} />

        <MyButtonCustom
          title={`+ Admit Student`}
          type="view"
          href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/pageAdmission`}
        />
      </div>
        ...
        >
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md px-2 text-center">
            <MyButtonCustom
              title='Assign'
              type='create'
              href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudentsUnAssigned/${item.id}`}
              />
          </div>
        </div>
      ))}
...
const SearchUserProfile = ({school_id, params}: any) => {
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (!value) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${school_id}/pageStudentsUnAssigned`)
    }
    if (field == "specialty") {
      redirect(`/${params.domain}/Section-H/pageAdministration/${school_id}/pageStudentsUnAssigned?specialty__main_specialty__specialty_name=${value}`)
    }
    if (field == "academic_year") {
      if (value && value.toString().length == 4){
        redirect(`/${params.domain}/Section-H/pageAdministration/${school_id}/pageStudentsUnAssigned?specialty__academic_year=${value}/`)
      }
      redirect(`/${params.domain}/Section-H/pageAdministration/${school_id}/pageStudentsUnAssigned?specialty__academic_year=${value}`)
    }
    if (field && value && value.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${school_id}/pageStudentsUnAssigned?user__${field}=${value}`)
    }
  }
  return (
    <>
