...
const page = async ({
    ...
    const apiSchoolInfo: any = await getData(protocol + "api" + params.domain + GetSchoolInfoUrl, { campus__id: params.school_id, fieldList: ["id", "school_name", "address", "town", "po_box", "email", "telephone_one", "niu", "region", "country", "website"] });
    const apiStudentInfo: any = await getData(protocol + "api" + params.domain + GetUserProfileUrl, { id: params.profile_id, fieldList: ["id", "user__matricle", "user__full_name", "user__sex", "user__dob", "user__email", "user__telephone", "specialty__main_specialty__specialty_name", "specialty__academic_year", "specialty__level__level", "specialty__main_specialty__field__field_name", "specialty__main_specialty__field__domain__domain_name"] });

    console.log(apiSchoolFees, 30)
    console.log(apiSchoolInfo, 31)
...
const GenerateTranscript = async ({ searchParams, params }: any) => {
    const apiData = await getData(protocol + "api" + params.domain + GetUserProfileUrl, {
