...
import { NotificationUrl } from '@/NoDomain/Utils-H/notiControl/notiConfig'
import { ActionCreate } from '@/serverActions/actionGeneral'
import { GetDomainUrl, GetSchoolInfoUrl, GetSpecialtyUrl } from '@/NoDomain/Utils-H/appControl/appConfig'
...
const page = async ({
    ...
    // const apiNotification: any = await getData(
    //     GetNotificationUrl, { ...searchParams, nopage: true },
    // );
...
const CreateAnnouncements = async ({ params, searchParams }: any) => {
    ...
    const apiSchool: any = await getData(protocol + GetSchoolInfoUrl, { nopage: true, fieldList: [ "id", "school_name", "town", "campus__name" ] },);
    const apiDomain: any = await getData(protocol + GetDomainUrl, { nopage: true },);
    const apiSpecialty: any = await getData(protocol + GetSpecialtyUrl, {
    ...
    const onSubmitServerAction = async (formData: FormData) => {
        ...
        const response = await ActionCreate(data, SchemaCreateEditNotifiation, protocol + NotificationUrl)
