...
const page = async ({
    ...
    const apiData: GetTransactionsInter[] | any = await getData(protocol + "api" + params.domain + GetTransactionUrl, {
    ...
    const apiStudentSchoolFees: GetTransactionsInter[] | any = await getData(protocol + "api" + params.domain + GetSchoolFeesUrl, {
        ...
        <LayoutAdmin>
            <>
                <Breadcrumb
                    ...
                    pageName1="Back"
                    link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.school_id}`}
                />

                {searchParams && <NotificationError errorMessage={searchParams} />}

                {apiData && apiStudentSchoolFees && <List apiData={apiData} params={params} searchParams={searchParams} schoolfees={apiStudentSchoolFees[0]} />}
...
const List: FC<ListProps> = async ({ apiData, params, searchParams, schoolfees }) => {
    ...
    const onSubmitServerAction = async (formData: FormData) => {
        ...
        const account = formData.get("reason");
        const payment_method = formData.get("payment_method");
        const operation_type = "income";
        const reason = account?.toString().toLowerCase();
        ...
        const response = await ActionCreate(data, SchemaCreateEditTransactions, protocol + "api" + params.domain + TransactionUrl)

        console.log(response, 92)

        if (response?.id) {
            redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/fees?created=Operation Successfull`)
        } else {
            redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/fees/create?error=Operation Failed`)
        }
    }
                        ...
                        <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
                            ...
                            <form className="bg-slate-50 dark:bg-slate-700 flex flex-col font-medium gap-4 md:gap-10 p-6.5 tracking-wide" action={onSubmitServerAction}>
                                ...
                                <div className="flex flex-col w-full">
                                        ...
                                        >
                                            <option key={0} value={""} className="dark:text-bodydark my-2 text-body">-----------</option>
                                            {ConfigData[params.domain]['higher'].registration_seperate_tuition ? <option key={1} value={"REGISTRATION"} className="dark:text-bodydark my-2 text-body">REGISTRATION</option> : null}
