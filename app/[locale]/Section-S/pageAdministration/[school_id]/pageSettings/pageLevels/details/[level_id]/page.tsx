import { GetLevelUrl, LevelUrl } from '@/NoDomain/Utils-S/appControl/appConfig';
import { GetLevelInter, LevelInter } from '@/NoDomain/Utils-S/appControl/appInter';
...
const EditPage = async ({
  ...
  const apiData: LevelInter | any = await getData(protocol + GetLevelUrl, { id: params.level_id });
...
const EditDelete: FC<EditDeleteProps> = ({ apiData, params }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.level_id, SchemaCreateEditLevel, protocol + LevelUrl, `/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels`)
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await DeleteAction(LevelUrl, params.level_id)
