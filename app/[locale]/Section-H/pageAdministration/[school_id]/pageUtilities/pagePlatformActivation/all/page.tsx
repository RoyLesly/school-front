    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Back"
          link1={`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation`}
        />
...
const List = async ({ apiData, params }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (!pay.operation && pay.transaction == "could-not-perform-transaction") {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/all?error=Transaction Cancelled By USer`)
    }
    if (!pay.operation && pay.transaction == "low-balance-payer") {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/all?error=Not Enough Funds`)
    }
    if (!pay.operation && pay.transaction == "ENOTFOUND") {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/all?error=Transaction Error`)
    }
    ...
    if (pay.operation) {
      ...
      for (let index = 0; index < apiData.length; index++) {
        ...
        const response = await ActionCreate({ ...data, schoolfees_id: element.id }, SchemaTransactionCreate, TransactionUrl)
        console.log(81, response)
        if (index == apiData.length - 1) {
          if (response.error && Object.keys(response.error).length > 0) {
            redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/all?error=${JSON.stringify(response.error)}`)
          }
          if (response.id) {
            redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation?success=SUCESSFULL ACTIVATION`)
          }
        }
      }
    } else {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/all?error=Transaction Failed`)
    }
  }
