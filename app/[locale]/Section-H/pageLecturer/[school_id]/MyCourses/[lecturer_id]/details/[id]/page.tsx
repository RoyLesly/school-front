...
const EditPage = async ({
  ...
  const apiData: any = await getData(`${GetCourseUrl}/${params.id}`, { fieldList: [ 
    "id", 
...
const EditDelete:FC<EditDeleteProps> = ( { apiData, params }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response.error) {
      redirect(`/pageLecturer/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response.errors) {
      redirect(`/pageLecturer/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.errors).replaceAll(" ", "-")}`)
    }
    if (response?.detail) {
      redirect(`/pageLecturer/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/pageLecturer/${params.school_id}/pageSettings/pageLevels?updated=Successfully-Updated-${JSON.stringify(response.level).replaceAll(" ", "-")}`)
    }
  }
