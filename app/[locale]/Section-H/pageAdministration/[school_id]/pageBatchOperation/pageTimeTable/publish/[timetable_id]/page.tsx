...
import { GetTimeTableUrl, TimeTableUrl } from '@/NoDomain/Utils-H/timeControl/timeConfig';
...
const page = async ({
  ...
  params: { school_id: string, timetable_id: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData = await getData(protocol + GetTimeTableUrl, { id: params.timetable_id, fieldList: [
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          link1={`/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/publish`}
        />
...
const PublishConfirm = ({ apiData, params, searchParams }: any) => {
  ...
  const onSubmitSearchAction = async () => {
    ...
    const response = await ActionEdit(data, data.id.toString(), SchemaCreateEditTimeTable, protocol + TimeTableUrl)

    if (response.id == apiData.id){
      if (value){
        redirect(`/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/publish?success=Published Successfully`)
      } else {
        redirect(`/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/publish?success=Closed Successfully`)
      }
    }
