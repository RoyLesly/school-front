...
import { SubjectUrl } from '@/Domain/Utils-S/appControl/appConfig'
    ...
    const submit = async () => {
        ...
        if (checkCode.length < 1 && checkCredit.length < 1 && checkType.length < 1 && pathname) {
            ...
            if (data && data.length > 0 && pn) {
                ...
                .then(res => {
                    ...
                    if (res && res.length > 0 && pn) {
                        ...
                        if (!t.includes("fulfilled")){
                            router.push(`/Section-H/pageAdministration/${pn[0]}/pageBatchOperation/pageSubjectAssignment?error=Operation Successful`);
                        } else {
                            router.push(`/Section-H/pageAdministration/${pn[0]}/pageBatchOperation/pageSubjectAssignment?created=Operation Successful`);
                        }
                        setLoading(false);
                    }
                })
                .catch(err => {
                    if (pn){
                        router.push(`/Section-H/pageAdministration/${pn[0]}/pageBatchOperation/pageSubjectAssignment?specialty_id=${searchParams.get("specialty_id")}&semester=${searchParams.get("semester")}&error=${JSON.stringify(err)}`);
                    }
                })
                return
            } else {
