...
import { CustomUserUrl, DepartmentUrl } from '@/Domain/Utils-H/userControl/userConfig';
import { DeptInter } from '@/Domain/Utils-H/userControl/userInter';
import { Metadata } from 'next';
import { redirect, RedirectType } from 'next/navigation';
import React from 'react'
import { SchemaCreateCustomUser, UserProfileLectAdminEditSchema } from '@/schemas-user';
...
const page = async ({
  ...
  const deptData: DeptInter[] | any = await getData(protocol + "api" + params.domain +DepartmentUrl, { ...searchParams, name: "Lecturer" });
  return (
    <LayoutAdmin>
      <>
        <Breadcrumb
          pageName="New Lecturer"
          pageName1="Lecturers"
          link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers`}
        />

        {searchParams && <NotificationError errorMessage={searchParams} />}

        {deptData && deptData.unauthorized && <SessionExpired />}

        {deptData && <Create deptData={deptData} params={params} />}
...
export const metadata: Metadata = {
  title:
    "Lecturer-Create",
  description: "This is Leturer Page",
};
...
const Create = async ({ params, deptData }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const data = {
      ...
      page: [],
      prefix: ConfigData[`${params.domain}`].prefix.method + ConfigData[`${params.domain}`].prefix.higher,
      created_campus: [parseInt(params.school_id)]
    }

    const response = await ActionCreate(data, SchemaCreateCustomUser, protocol + "api" + params.domain + CustomUserUrl, `/pageAdministration/${params.school_id}`)
    ...
    if (response?.id) {
      if (response.id) {
        ...
        const response2 = await ActionCreate(userProfileData, UserProfileLectAdminEditSchema, protocol + "api" + params.domain + UserProfileUrl)
        if (response2.id) {
          redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers?created=Successfully Created`, RedirectType.replace)
        }
      }
    }
    // console.log(response)
    if (response?.error) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response.error)}`)
    }
    if (response?.detail) {
      console.log(80, response.detail)
    }
    if (response) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response)}`)
    }
  }
