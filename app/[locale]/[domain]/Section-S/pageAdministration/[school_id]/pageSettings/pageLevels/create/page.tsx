import { LevelUrl } from '@/Domain/Utils-S/appControl/appConfig';
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          link2={`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels`}
        />
...
const Create = async ({ params }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionCreate(data, SchemaCreateEditLevel, protocol  + "api" + params.domain + LevelUrl, `/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels`)
    
    if (response.error) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response.errors) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.errors).replaceAll(" ", "-")}`)
    }
    if (response?.detail) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels?created=Successfully-Created-${JSON.stringify(response.level).replaceAll(" ", "-")}`)
    }
    if (response) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageLevels/create?error=${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
