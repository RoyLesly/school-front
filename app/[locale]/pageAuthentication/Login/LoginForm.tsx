...
const LoginForm = () => {
  ...
  useEffect(() => {
    ...
    if (count == 3) {
      const getSchools = async () => {
        var response = await getData(protocol + GetSchoolInfoUrl, { nopage: true, fieldList: ["id", "school_type", "school_name", "campus__id", "campus__name", "campus__region"] })
    ...
    if (count == 5) {
      ...
      // IF ASSIGNED TO 1 CAMPUS (Admin and Lecturers)
      ...
      // IF MULTIPLE CAMPUSES (Admin and Lecturers
  ...
  const onSubmitServerAction = async (prevState: any, formData: FormData) => {
    ...
    const response = await ActionLogin(data, protocol + LoginUrl)
        ...
        <div className="bg-gray dark:bg-gray-800 dark:border dark:border-gray-700 md:mt-0 rounded-lg shadow sm:max-w-md w-full xl:p-0">
          <div className="flex flex-col p-6 sm:p-8">
            ...
            <div className="flex items-center justify-between">
              <a href={`https://wa.me/+237${ConfigData.help_number}/?text=Enter%20Matricle:%20OR%20Username:%20Complain: `} className="bg-slate-200 dark:text-primary-500 font-medium hover:underline px-4 py-1 rounded text-slate-700 text-sm">Help</a>
