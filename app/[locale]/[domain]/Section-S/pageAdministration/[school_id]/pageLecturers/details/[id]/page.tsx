...
import { ActionEdit, ActionDelete } from '@/Domain/Utils-S/serverActions/actionGeneral';
import { CustomUserUrl, DepartmentUrl, SecondaryProfileUrl } from '@/Domain/Utils-S/userControl/userConfig';
import { CustomUserInter, DeptInter } from '@/Domain/Utils-S/userControl/userInter';
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Settings"
          pageName2="Lecturer"
          link1="/Section-H/pageAdministration/pageLectruers"
        />
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, id, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response?.detail) {
      redirect(`/Section-H/pageAdministration/pageLecturers/details?error=Error-${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/Section-H/pageAdministration/pageLecturers?updated=Updated-${JSON.stringify(response.full_name).replaceAll(" ", "-")}`)
    } else {
      redirect(`/Section-H/pageAdministration/pageLecturers/details/${id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(SecondaryProfileUrl, apiData.user.id)
