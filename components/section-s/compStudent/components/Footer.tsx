    ...
    <div className="bottom-0 fixed flex flex-col h-[62px] justify-between w-full">
      ...
      <nav className='bg-blue-900 flex items-center justify-between left-0 px-10 py-2 w-full'>
        {footerIcons.map((item) => {
          var link: string = ""
          if (item.link == "student") { link = `/pageStudent/${params.userprofile_id}/${params.specialty_id}` }
          if (item.link == "settings") { link = `/pageStudent/${params.userprofile_id}/${params.specialty_id}/Settings` }
          if (item.link == "profile") { link = `/pageStudent/${params.userprofile_id}/${params.specialty_id}/Profile` }
          return (
