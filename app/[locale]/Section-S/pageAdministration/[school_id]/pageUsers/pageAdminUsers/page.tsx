...
import { CustomUserInter, GetCustomUserInter } from '@/NoDomain/Utils-S/userControl/userInter'
...
const page = async ({
  ...
  const apiData: GetCustomUserInter[] | any = await getData(protocol + GetCustomUserUrl, { ...searchParams, nopage: true, role: "admin", school__campus__id: params.school_id, is_staff: false });
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          link2={`/Section-S/pageAdministration/${params.school_id}/pageUsers`}
        />
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex gap-2 justify-between md:px-3 md:py-3 px-2 py-1 xl:px-7.5">
        ...
        <div className='flex flex-row gap-2'>
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/pageAdministration/${school_id}/pageUsers/pageStutsUsers`}>Students</Link></button>
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/pageAdministration/${school_id}/pageUsers`}>Lecturers</Link></button>
        </div>
      ...
      {apiData && apiData.length > 0 && apiData.map((item: CustomUserInter, key: number) => (
        ...
        >
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md">
            <MyButtonCustom
              title='View'
              href={`/pageAdministration/${school_id}/pageUsers/details/${item.id}`}
              type="view"
            />
          </div>
        </div>
      ...
      <MyPagination
        ...
        count={apiData.count}
        thisUrl={`/pageAdministration/${school_id}/pageUsers`}
      />
...
const SearchUser = ({ school_id }: any) => {
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (full_name) {
      redirect(`/pageAdministration/${school_id}/pageUsers?full_name=${full_name}`)
    }
  }
  ...
  return (
    <>
      ...
      <form action={onSearchDrugServerAction} className='flex flex-row gap-1'>
        ...
        <Link className='flex items-center justify-center' href={`/pageAdministration/${school_id}/pageUsers`}><FiRefreshCw size={23} /></Link>
      </form>
