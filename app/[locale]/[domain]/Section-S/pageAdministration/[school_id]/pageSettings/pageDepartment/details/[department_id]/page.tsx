...
import { DepartmentUrl, GetDepartmentUrl } from '@/Domain/Utils-S/userControl/userConfig';
...
const EditPage = async ({
  ...
  params: { school_id: string, department_id: string , domain: string};
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData: GetDepartmentInter | any = await getData(protocol  + "api" + params.domain + GetDepartmentUrl, { id: params.department_id, nopage: true });
...
const EditDelete: FC<EditDeleteProps> = ({ apiData, params }) => {
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(DepartmentUrl, params.level_id)
