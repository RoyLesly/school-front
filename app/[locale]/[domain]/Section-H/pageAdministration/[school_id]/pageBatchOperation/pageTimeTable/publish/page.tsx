...
import { GetTimeTableInter } from '@/Domain/Utils-H/timeControl/timeInter'
import { GetTimeTableUrl } from '@/Domain/Utils-H/timeControl/timeConfig'
...
const page = async ({
  ...
  const apiTimeTables: any = await getData(protocol + "api" + params.domain + GetTimeTableUrl, { specialty__school_campus__id: params.school_id, fieldList: [
...
const List = async ({ apiTimeTables, params }: any) => {
  ...
  return <div className='flex flex-col flex-grow'>
    ...
    {apiTimeTables || apiTimeTables.count ? apiTimeTables.results.map((item: GetTimeTableInter) => <div key={item.id} className="dark:border-strokedark dark:odd:bg-slate-600 dark:text-white grid grid-cols-4 md:grid-cols-6 odd:bg-notiblue p-2 px-4 text-black text-lg tracking-wider">
      ...
      <div className="hidden items-center justify-center md:flex">
        {item.publish ? 
        <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/publish/${item.id}?publish=false`}><GrStatusGood color='green' size={30} /> </Link>
        : 
        <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/publish/${item.id}?publish=true`}>X</Link>}
