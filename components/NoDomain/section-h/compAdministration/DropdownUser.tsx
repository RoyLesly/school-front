...
import { SchoolInfoInter } from "@/NoDomain/Utils-H/appControl/appInter";
  ...
  useEffect(() => {
    if (count == 0) {
      ...
      const getSchl = async () => {
        ...
        var res = await getData(protocol + GetSchoolInfoUrl, { id: sch, nopage: true, fieldList: [ "id", "school_type" ]})
      ...
      <div className="relative">
        ...
        >
          ...
          {school && user && user.dept && user.dept.map((item: string) => {
               ...
               return <li key={item}>
                <Link
                href={`/${school.school_type}/page${fil}/${school.id}?id=${user.id}`}
