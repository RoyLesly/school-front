...
const page = async ({
  ...
  const coursesData: any = await getData(protocol + GetCourseUrl, { ...searchParams, nopage: true, assigned_to__id: params.lecturer_id, specialty__school__campus__id: params.school_id, fieldList: [
    "id", 
        ...
        {apiData && apiData.length > 0 ? apiData.map((item: GetCourseInter, key: number) => (
          ...
          >
            ...
            <div className="col-span-3 flex items-center">
              ...
              <Link href={`/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}/details/${item.id}`} className="flex md:hidden underline">
                {item.main_course__course_name}
              </Link>
            </div>
            <div className="col-span-3 hidden items-center md:flex">
            ...
            <div className="gap-2 hidden items-center justify-end md:flex md:gap-6 w-full">
              <MyButtonCustom
                title="View"
                href={`/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}/details/${item.id}`}
              />
            </div>
          </div>
        ))
      :
      <div className='flex font-semibold italic items-center justify-center md:py-60 py-40 text-xl tracking-widest'>No Courses Found</div>}
...
const SearchLecturerCourses = async ({params}: any) => {
  ...
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (value && value.toString().length > 1) {
      redirect(`/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}?main_course__course_name=${value}&specialty__academic_year=${year}&specialty__level__id=${level}`)
    } 
    if (year || level) {
      redirect(`/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}?specialty__academic_year=${year}&specialty__level__id=${level}`)
    }
    redirect(`/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}`)
