...
const DesktopView = ({ apiSpecialty, data, yearweek, params }: any) => {
        ...
        <div className='bg-white hidden md:flex md:flex-col text-black'>
            ...
            </div>
            {
                [...new Set(yearweek)].map((week: any) => <div key={week} className='bg-blue-800 gap-10 mb-10 p-2 rounded'>
                    <h1 className='font-semibold text-center text-lg text-white tracking-widest w-full'>{getStartEndOfWeek(week.slice(2, 4), new Date().getFullYear(), "join")}</h1>
                    {Days.map((day: string) => <div key={day} className='bg-blue-100 flex flex-row gap-2 items-center my-2 p-1 rounded'>
                        <div className='font-bold text-[16px] text-center tracking-widest w-[15%]'>{day}</div>
                        ...
                        {data.filter((item: GetTimeSlotInter) => item.timetableday__timetableweek__year_week == week && item.timetableday__day == day).map(
                            (item: GetTimeSlotInter) => <div key={item.id} className='bg-white border col-span-3 flex my-1 px-2 py-1 rounded w-full'>
                                <span className='text-sm w-[15%]'>Day-{item.start.slice(8, 10)} {item.start.slice(11, 16)}</span>
                                <span className={`${item.status == "CHECKED-IN" ? "bg-blue-300": item.status == "CHECKED-OUT" ? "bg-green-300" : ""} px-1 rounded font-medium italic w-[83%]`}>{item.title}</span>
                            </div>
                        )}
                        </div>
