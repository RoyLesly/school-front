    ...
    useEffect(() => {
        if (count == 0){
            const fetchAcademicYear = async () =>{
                try {
                  const {data: response} = await axios.get(protocol + "api" + domain + AcademicYearUrl);
        ...
        if (count == 2 && academicYear){
            router.push(`/Section-HpageAdministration/pageResult/pagePublish?academic_year=` + academicYear);
            setCount(1)
        }
    }, [ count, academicYear, router, domain ])
