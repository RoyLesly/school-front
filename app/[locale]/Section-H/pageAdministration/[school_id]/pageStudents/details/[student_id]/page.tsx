...
import { GetUserProfileInter, ProgramInter } from '@/NoDomain/Utils-H/userControl/userInter';
import { CustomUserUrl, GetUserProfileUrl, ProgramUrl, UserProfileUrl } from '@/NoDomain/Utils-H/userControl/userConfig';
...
const page = async ({
  ...
  const apiData: any = await getData(GetUserProfileUrl, {id: params.student_id, fieldList: [
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, school_id, student_id, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, apiData.id, UserProfileStudentEditSchema, UserProfileUrl, `/pageAdministration/${school_id}`)

    console.log(response)
    if (response.error) {
      redirect(`/Setion-H/pageAdministration/${school_id}/pageStudents/detail/${student_id}?error=${JSON.stringify(response.error)}`)
    }
    if (response.errors) {
      redirect(`/Setion-H/pageAdministration/${school_id}/pageStudents/detail/${student_id}?error=${JSON.stringify(response.errors)}`)
    }
    if (response?.detail) {
      redirect(`/Setion-H/pageAdministration/${school_id}/pageStudents/detail/${student_id}?error=${JSON.stringify(response.detail)}`)
    }
    if (response?.id) {
      redirect(`/Setion-H/pageAdministration/${school_id}/pageStudents?updated=Updated Student Info`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(UserProfileUrl, student_id)
