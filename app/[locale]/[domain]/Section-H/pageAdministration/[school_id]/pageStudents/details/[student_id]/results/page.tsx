...
const EditDelete: FC<EditDeleteProps> = async ({ apiStudentInfo, params }) => {
  ...
  const apiMyResultsI: any = await getData(protocol + "api" + params.domain + GetResultUrl, { active: true, student__id: apiStudentInfo.id, course__specialty__id: apiStudentInfo.specialty__id, course__semester: "I", fieldList: ["id", "course__main_course__course_name", "course__assigned_to__full_name", "course__semester", "ca", "exam", "resit", "average", "validated"] });
  const apiMyResultsII: any = await getData(protocol + "api" + params.domain + GetResultUrl, { active: true, student__id: apiStudentInfo.id, course__specialty__id: apiStudentInfo.specialty__id, course__semester: "II", fieldList: ["id", "course__main_course__course_name", "course__assigned_to__full_name", "course__semester", "ca", "exam", "resit", "average", "validated"] });
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      ...
      <div className='flex items-center justify-between px-10'>
        ...
        {/* { apiPaymentInfo && apiPaymentInfo.count == 1 && apiPaymentInfo.results[0].platform_paid && <div className='flex font-medium gap-4 items-center justify-center my-2'>
          <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/editresults`} className='bg-teal-700 font-medium px-10 py-2 rounded text-[18px] text-white'>Edit Result</Link>
        </div>} */}
        { true && <div className='flex font-medium gap-4 items-center justify-center my-2'>
          <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/editresults`} className='bg-teal-700 font-medium px-10 py-2 rounded text-[18px] text-white'>Edit Result</Link>
        </div>}
      </div>
      ...
      {apiMyResultsI && apiMyResultsI.count && <div className='flex flex-col p-2'>
        ...
        {true ? apiMyResultsI.results.map((item: GetResultInter, index: number) => <div key={item.id}
        ...
        >
          ...
          <div className="col-span-2 hidden items-center justify-center lg:flex">
            <span className="font-medium text-sm">{item.course__assigned_to__full_name}</span>
          </div>
        </div>)
          :
