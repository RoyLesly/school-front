    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          pageName="New Admin"
          pageName1="Lecturers"
          link1={`/Section-H/pageAdministration/${params.school_id}/pageUsers`}
        />
...
const Create = async ({ searchParams, params }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionCreate(data, SchemaCreateCustomUser, protocol + "api" + params.domain + CustomUserUrl, `/Section-H/pageAdministration/${params.school_id}`)

    console.log(response)
    if (response?.error) {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageLecturers/Section-H/pageAdmission?error=${JSON.stringify(response.error)}`)
    }
    if (response?.id) {
    ...
    if (response) {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageLecturers/Section-H/pageAdmission?error=${JSON.stringify(response)}`)
    }
  }
