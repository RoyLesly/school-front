...
const page = async ({
  ...
  const acadYears = await getData(protocol + AcademicYearUrl, { nopage: true, fieldList: ["level"] })
  ...
  const userCardData = sortedAcadYears && await getData(protocol + GetDashUserCardUrl, {
  ...
  const profileSexChartData = sortedAcadYears && await getData(protocol + GetDashProfileSexChartUrl, {
    academic_year: searchParams && Object.keys(searchParams).includes("academic_year") ? searchParams.academic_year : sortedAcadYears[0],
    this_year: searchParams && Object.keys(searchParams).includes("this_year") ? searchParams.this_year : today.getFullYear()
  })
  const specialtyLevelCountChartData = sortedAcadYears && await getData(protocol + GetDashSpecialtyLevelCountChartUrl, {
    academic_year: searchParams && Object.keys(searchParams).includes("academic_year") ? searchParams.academic_year : sortedAcadYears[0],
    this_year: searchParams && Object.keys(searchParams).includes("this_year") ? searchParams.this_year : today.getFullYear()
  })
