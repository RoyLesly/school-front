...
import { GetCourseUrl, GetPublishUrl } from '@/Domain/Utils-H/appControl/appConfig';
...
const EditPage = async ({
  ...
  const course = await getData(protocol + "api" + params.domain + GetCourseUrl + "/" + params.id, { fieldList: [ "id", "semester", "main_course__course_name", "specialty__id", "specialty__main_specialty__specialty_name" ]})
...
const Select:FC<SelectProps> = async ( { params, course }) => {
  ...
  const publish = await getData(protocol + "api" + params.domain + GetPublishUrl, { specialty__id: course.specialty__id, fieldList: [ "id", "specialty__id", "semester", "ca", "portal_ca", "portal_exam", "portal_resit" ]})
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark flex flex-col gap-10 md:gap-20 py-20 rounded-sm shadow-default">
      ...
      { publish.count == 1 && publish.results[0].semester == "I" && course.semester == "I" && <div className='flex flex-col gap-10 items-center justify-center md:flex-row md:py-10 px-10'>
        {publish.results[0].portal_ca && <Link 
          ...
          href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${params.id}/FillCA`}
        >
            Sem I - CA
          </Link>}
        {publish.results[0].portal_exam && <Link 
          className='bg-blue-800 flex font-semibold h-20 items-center justify-center md:h-24 rounded text-center text-white text-xl tracking-wide w-48'
          href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${params.id}/FillExam`}
        >
            Sem I - EXAM
          </Link>}
        {publish.results[0].portal_resit && <Link 
          className='bg-blue-800 flex font-semibold h-20 items-center justify-center md:h-24 rounded text-center text-white text-xl tracking-wide w-48'
          href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${params.id}/FillResit`}
        >
            Sem I - RESIT
          </Link>}
      </div>}
