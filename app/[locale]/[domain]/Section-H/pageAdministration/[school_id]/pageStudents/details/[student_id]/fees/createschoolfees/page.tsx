...
import { CustomUserInter, GetUserProfileInter } from '@/Domain/Utils-H/userControl/userInter';
...
const page = async ({
    ...
    const student = await getData(protocol + "api" + params.domain +  GetUserProfileUrl, { id: params.student_id, nopage: true, fieldList: [ 
...
const List: FC<ListProps> = async ({ apiData, params, searchParams }) => {
    ...
    const onSubmitServerAction = async (formData: FormData) => {
        ...
        const response = await ActionCreate(data, SchemaCreateEditSchoolFees, protocol + "api" + params.domain + SchoolFeesUrl)
