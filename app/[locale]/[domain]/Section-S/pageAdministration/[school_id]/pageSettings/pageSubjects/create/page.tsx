import { SubjectUrl, GetClassroomUrl, GetMainSubjectUrl } from '@/Domain/Utils-S/appControl/appConfig';
...
import { SchemaCreateEditSubject } from '@/Domain/Utils-S/schemas/schemas';
...
const page = async ({
  ...
  const classroomData: any = await getData(protocol  + "api" + params.domain + GetClassroomUrl, { ...searchParams, nopage: true, fieldList: ['id', "level__level", "level__option", "academic_year"] })
  const mainSubjectData: any = await getData(protocol  + "api" + params.domain + GetMainSubjectUrl, { nopage: true, fieldList: ["id", "subject_name"] })
  const lecturerData: any = await getData(protocol  + "api" + params.domain + GetCustomUserUrl, { role: "teacher", nopage: true, is_active: true, is_staff: false, school__campus__id: params.school_id });
...
const Create = async ({ params, classroomData, mainSubjectData, lecturerData, searchParams }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionCreate(data, SchemaCreateEditSubject, SubjectUrl, `/Section-S/pageAdministration/${params.school_id}/pageSettings/pageSubjects`)
