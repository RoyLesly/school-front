...
const Create = async ({ searchParams, params }: any) => {
  ...
  const deptData: DeptInter[] | any = await getData(protocol + DepartmentUrl, { ...searchParams, name: "Administration" });
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    console.log(67, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxddddddddddddddddddddddddddddddddddddddddd")
    ...
    const response = await ActionCreate(data, SchemaCreateCustomUser, CustomUserUrl, `/Section-H/pageAdministration/${params.school_id}`)
    ...
    if (response?.id) {
      if (response.id) {  
        ...
        const response2 = await ActionCreate(userProfileData, UserProfileLectAdminEditSchema, UserProfileUrl)
        if (response2.id) {
