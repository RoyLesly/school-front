...
import { GetProgramUrl, ProgramUrl } from '@/NoDomain/Utils-H/userControl/userConfig';
...
const EditPage = async ({
  ...
  const apiData: ProgramInter | any = await getData(`${protocol + GetProgramUrl}/${params.program_id}`, { ...searchParams });
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.program_id, SchemaCreateEditProgram, protocol + ProgramUrl, `/Section-H/pageAdministration/${params.school_id}/pageSettings/pagePrograms`)
