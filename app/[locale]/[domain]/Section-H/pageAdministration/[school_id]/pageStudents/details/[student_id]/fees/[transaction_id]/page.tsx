...
import {  } from '@/Domain/UserControl/userInter';
import { Metadata } from 'next';
import Link from 'next/link';
import React, { FC } from 'react'
import NotificationError from '@/section-h/common/NotificationError';
import { GetTransactionUrl } from '@/Domain/Utils-H/feesControl/feesConfig';
...
const page = async ({
  ...
  params: { school_id: string,  domain: string, student_id: string, transaction_id: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {


  const apiData: GetTransactionsInter[] | any = await getData(protocol + "api" + params.domain + GetTransactionUrl, { id: params.transaction_id, nopage: true, fieldList: [ 
    "id", "reason", "payment_method", "amount", "schoolfees__userprofile__user__full_name", "schoolfees__balance", "payer_name", "telephone", "status", 
    "ref", "schoolfees__userprofile__specialty__tuition", "operator", "created_at", "created_by__full_name",
  ] });
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Back"
          link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/fees`}
        />

        <NotificationError errorMessage={searchParams} />

        {apiData ? <List apiData={apiData[0]} params={params} /> 
          : 
...
interface ListProps {
  params: any
  apiData: GetTransactionsInter | any
}
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark flex flex-col items-center justify-center md:flex-row md:gap-16 md:px-20 md:py-40 rounded shadow-default">
      ...
      {apiData.length > 0 && apiData.map((item: GetTransactionsInter, index: number) => <div key={item.id} className="border-stroke border-t dark:border-strokedark dark:text-white font-semibold grid grid-cols-3 md:grid-cols-8 md:px-6 md:text-lg px-4 py-2">
        ...
        <div className="flex items-center justify-center text-center">
          <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/fees/${item.id}`} className="font-medium">Details</Link>
        </div>
      </div>)}
