...
import { GetResultTranscriptUrl, GetSchoolInfoUrl } from '@/NoDomain/Utils-S/appControl/appConfig'
import { GetSecSchoolFeesUrl } from '@/NoDomain/Utils-S/feesControl/feesConfig'
import { redirect } from 'next/navigation'
import TransLayoutTerm from './TransLayoutTerm'
...
const page = async ({
    ...
    const apiSchoolFees: any = await getData(protocol + GetSecSchoolFeesUrl, { SecondaryProfile__id: params.profile_id, nopage: true, fieldList: ["id", "platform_charges", "platform_paid", "balance"] });
    const apiSchoolInfo: any = await getData(protocol + GetSchoolInfoUrl, { campus__id: params.school_id, nopage: true, fieldList: ["id", "school_name", "address", "town", "po_box", "email", "telephone_one", "niu", "region", "country", "website"] });
    ...
    if (apiStudentInfo && apiStudentInfo.unauthorized) {
        ...
        return (
            <LayoutAdmin>
                <>
                    <Breadcrumb
                        pageName={`Generate Term-${searchParams && searchParams.term } Report Card`}
                        pageName1="Back"
                                ...
                                apiSchoolFees && apiSchoolFees.length > 0 ?
                                    apiSchoolFees[0].platform_paid ?
                                        <div className='flex font-semibold items-center justify-center py-40 text-white text-xl tracking-widest'>
                                            ...
                                            <Link href={`/pageAdministration/${params.school_id}/pageUtilities/pagePlatformActivation/${apiSchoolFees[0].id}`} className="bg-bluedark px-6 py-1 rounded text-white">Activate</Link>
                                        </div>
                                        :
                                        apiMyResults ?
                                            apiStudentInfo && apiSchoolInfo && apiStudentInfo.length > 0 && apiSchoolInfo.length > 0 && searchParams ? 
                                                <TransLayoutTerm term={searchParams.term} results={apiMyResults} schoolinfo={apiSchoolInfo[0]} studentInfo={apiStudentInfo[0]} /> 
                                                : 
