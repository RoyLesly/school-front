...
import { AcademicYearUrl, GetPublishUrl, GetResultUrl } from '@/NoDomain/Utils-H/appControl/appConfig';
...
const EditPage = async ({
  ...
  params: { school_id: string, lecturer_id: string, course_id: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiAcademicYear: any = await getData(protocol + AcademicYearUrl, { ...searchParams })
  const apiResult: any = await getData(protocol + GetResultUrl, { ...searchParams, course__id: params.course_id, course__semester: searchParams?.sem, nopage: true, fieldList: ["id", "course__specialty__id", "student__id"] })
// console.log(apiResult, 24)
  return (
    <LayoutAdmin>
      <>
        <Breadcrumb
        ...
        />
          ...
          <div className='bg-black flex flex-col font-semibold gap-10 items-center justify-center py-32 rounded text-2xl text-white'>
            ...
            <div><Link href={`/Section-H/pageAdministration/${params.school_id}/pageResult/create?sub=${params.course_id}`} className='bg-bluedark my-2 p-2 px-6 rounded'>Generate Results</Link></div>
          </div>
        }
...
const Select: FC<SelectProps> = async ({ params, searchParams, apiYear, specialty_id, results }) => {
  ...
  const apiSpecialtyStudentsCount: any = await getData( protocol +  GetUserProfileUrl, { ...searchParams, nopage: true, specialty__id: specialty_id, fieldList: ["id", "user__full_name"] })
  ...
  const apiDataPublish: any = await getData(protocol + GetPublishUrl, {
      ...
      {searchParams.sem == "I" && <div className='gap-10 grid grid-cols-3 items-center justify-center py-16'>
        {LinkOne.map((item: any) => (
          <Link
            ...
            key={item.id} href={`/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry/details/${params.course_id}/${item.page}`}
          >
            {item.title}
          </Link>
        ))}
