...
const MarksForm = ({ resultData, params }: any) => {
    ...
    const FieldChanged = (e: any, result: GetResultInter) => {
        if (e.name == "ca" && e.value > ConfigData.ca_limit){ return }
        if (e.name == "exam" && e.value > ConfigData.exam_limit){ return }
        ...
        if (ArrayMarksToSubmit && ArrayMarksToSubmit.length > 0) {
            ...
            .then(res => {
                ...
                if (res && res.length > 0) {
                    ...
                    if (t[0] == "fulfilled"){ router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/results?updated=SUCCESSFULLY !!!`); }
                    else { router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${params.student_id}/editresults?error=${t[0]}`); }
                }
            })
            .catch(err => {
                console.log(98, err)
