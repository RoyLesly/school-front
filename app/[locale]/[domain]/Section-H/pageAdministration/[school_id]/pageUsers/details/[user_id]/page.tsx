...
import { CustomUserUrl, GetCustomUserUrl } from '@/Domain/Utils-H/userControl/userConfig';
...
const page = async ({
  ...
  params: { school_id: string,  domain: string, user_id: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData: CustomUserInter | any = await getData(protocol + "api" + params.domain + GetCustomUserUrl, { nopage: true, id: params.user_id });
  ...
  return (
    <LayoutAdmin>
      <>
        ...
        <div>
          {apiData && apiData.length == 1 && <EditDelete apiData={apiData[0]} params={params} searchParams={searchParams} />}
        </div>
      </>
    </LayoutAdmin>
  )
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.user_id, SchemaEditCustomUser, protocol + "api" + params.domain + CustomUserUrl)
    console.log(response, 84)
    return

    if (response?.detail) { }
    if (response?.id) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers?updated=Updated-${JSON.stringify(response.full_name).replaceAll(" ", "-")}`)
    } else {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/details/${params.user_id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
