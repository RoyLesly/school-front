'use client';
import { MainCourseInter } from '@/NoDomain/Utils-H/appControl/appInter';
import React, { useState } from 'react';

const SearchMainCourse = ({ mainCourseData }: any) => {
    const [ newData, setNewData ] = useState<MainCourseInter[]>(mainCourseData);
    const onSearch = (text: string) => {
        if (text.length > 2){
            const fil = mainCourseData.filter((item: MainCourseInter) => item.course_name.toLowerCase().includes(text.toLowerCase()))
            setNewData(fil)
        }
        else { setNewData(mainCourseData)}
    }
    ...
    return (
        <div>
            ...
            >
                ...
                {newData && newData.map((item: MainCourseInter) => (<option key={item.id} value={item.id} className="dark:text-bodydark my-2 text-body">
                    {item.course_name}
                    </option>)
                )}
            </select>
...
export default SearchMainCourse