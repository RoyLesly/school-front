...
import { DayProgramUrl } from '@/Domain/Utils-H/timeControl/timeConfig'
import { DayProgramInter } from '@/Domain/Utils-H/timeControl/timeInter'
import SessionExpired from '@/section-h/common/SessionExpired'
import { ActionEdit } from '@/serverActions/actionGeneral'
import { SchemaCreateEditDayProgram } from '@/Domain/schemas/schemas'
...
const page = async ({
  ...
  params: { school_id: string, specialty_id: string, dayprogram_id: string, domain: string };
    ...
    <LayoutAdmin>
      <>
        {apiSpecialty && apiSpecialty.count && <Breadcrumb
          ...
          link2={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/${params.specialty_id}/listCourses`}
        />}
...
const List = async ({ data, params, specialty }: any) => {
  ...
  const SubmitUpdate = async (t: FormData) => {
    ...
    const response = await ActionEdit(updatedData, dayData.id, SchemaCreateEditDayProgram, protocol + "api" + params.domain + DayProgramUrl, `/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable`)
    
    if (response?.id) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/${params.specialty_id}/listCourses?updated=Successfully-Updated`)
    }
    redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/${params.specialty_id}/edit/${params.dayprogram_id}?error?error=Error-Updating`)
  }
