...
import { CustomUserUrl, DepartmentUrl, UserProfileUrl } from '@/Domain/Utils-H/userControl/userConfig';
...
export const metadata: Metadata = {
  title: "Leturer-Create",
  description: "This is Leturer Page",
};

const Create = async ({ searchParams, params }: any) => {

  const deptData: DeptInter[] | any = await getData(protocol + "api" + params.domain + DepartmentUrl, { ...searchParams, name: "Administration" });
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionCreate(data, SchemaCreateCustomUser, protocol + "api" + params.domain + CustomUserUrl, `/${params.domain}/Section-H/pageAdministration/${params.school_id}`)

    console.log(response, 89999)
    if (response?.id) {
      console.log(response.id, 91111)
      if (response.id) {  
        var userProfileData = {
          user_id: response.id,
        }
        ...
        console.log(response2, 9898)
        if (response2.id) {
          redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers?created=Successfully Created`)
        }
      }
    }
    if (response?.error) {
