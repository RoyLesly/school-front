import { GetLevelUrl, ClassroomUrl } from '@/Domain/Utils-S/appControl/appConfig';
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Settings"
          pageName2="Back"
          link2={`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms`}
        />
...
const Create = async ({ params }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionCreate(data, SchemaCreateEditClassroom, protocol + ClassroomUrl, `/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms`)

    console.log(response, 81)
    if (response.error) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/create?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.errors) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/create?error=${JSON.stringify(response.errors).replaceAll(" ", "-")}`)
    }
    if (response?.detail) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/create?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms?created=Successfully-Created-${JSON.stringify(response.id).replaceAll(" ", "-")}`)
    }
  }
