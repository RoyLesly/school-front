...
import { AcademicYearUrl, CourseUrl, DomainUrl, LevelUrl, SpecialtyUrl } from '@/NoDomain/Utils-H/appControl/appConfig';
import { CourseInter, DomainInter, LevelInter, SpecialtyInter } from '@/NoDomain/Utils-H/appControl/appInter';
    ...
    useEffect(() => {
        if (count == 0){
            const fetchDomains = async () =>{
                try {
                  const {data: response} = await axios.get(protocol + DomainUrl);
        ...
        if (count == 2 && academicYear && domain && level){
            const fetchSpecialties = async () =>{
                try {
                  const {data: response} = await axios.get(protocol + SpecialtyUrl + `?main_specialty__field__domain__id=${domain.id}&&academic_year=${academicYear}&&level__level=${level.level}`);
        ...
        if (count == 4 && specialty && semester){
            const fetchCourses = async () =>{
                try {
                  const {data: response} = await axios.get(protocol + CourseUrl + `?specialty__id=${specialty.id}&&semester=${semester}`);
        ...
        if (count == 6 && academicYear && domain && semester && specialty && course ){
            router.push(`/Section-H/pageAdministration/pageResult?course__id=` + course.id);
            setCount(7)
        }
