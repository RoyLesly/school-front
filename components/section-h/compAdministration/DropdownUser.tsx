...
import { SchoolInfoInter } from "@/Domain/Utils-H/appControl/appInter";
...
const DropdownUser = () => {
  ...
  const [school, setSchool] = useState<SchoolInfoInter>()
  ...
  useEffect(() => {
    if (count == 0 && domain) {
      ...
      const getSchl = async () => {
        var sch = localStorage.getItem("school")
        var res = await getData(protocol + "api" + domain + GetSchoolInfoUrl, { id: sch, nopage: true, fieldList: [ "id", "school_type" ]})
        if (res && res.length == 1){ setSchool(res[0])}
      }
      getSchl()
      if (session) {
        ...
        if (token){
          if (new Date().toISOString().toString().slice(0, 16) > new Date(token.exp * 1000).toISOString().slice(0, 16)){ 
            ...
            router.push("/pageAuthentication/pageSessionExpired")
          } else {
            setUser({ 
              id: token.user_id ? token.user_id.toString() : "", 
              username: token.username, 
              role: token.role, 
              dept: token.dept, 
              page: token.page, 
              school: token.school, 
              exp: new Date(token.exp * 1000).toISOString().slice(1, 17) })
          }
        }
      } else {
        router.push("/pageAuthentication/pageNotLoggedIn")
      }
      setCount(1)
    }
  }, [count, user, router, domain])


  const DeptLinkList: string[] = [
    '/pageAccounting',
    `/pageAdministration`,
    '/pageLecturer',
    '/pageStudent',
    ...
    <>
      <div className="relative">
        <Link
          ...
          href="#"
        >
          {user && <span className="hidden lg:block text-right">
        ...
        >
          ...
          {school && user && user.dept && user.dept.map((item: string) => {
               const fil = DeptLinkList.filter((link: string) => link.toLowerCase().includes(item.toLowerCase()))[0]
               return <li key={item}>
                <Link
                href={`/${domain}/${school.school_type}${fil}/${school.id}?id=${user.id}`}
            ...
            <li>
              <Link
                href={`/pageAdministration/pageMySettings/${user?.id}`}
                className="duration-300 ease-in-out flex font-medium gap-3.5 hover:text-primary items-center lg:text-base text-sm"
              >
                <svg
                  className="fill-current"
          ...
          <button onClick={() => router.push("/pageAuthentication/Logout")} className="duration-300 ease-in-out flex font-medium gap-3.5 hover:text-primary items-center lg:text-base px-6 py-4 text-sm">
            <svg
              className="fill-current"
              width="22"
              height="22"
