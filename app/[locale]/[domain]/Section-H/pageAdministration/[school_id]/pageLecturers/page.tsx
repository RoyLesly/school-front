...
const StudentList = async ({
  ...
  const apiDataLect: any[] | any = await getData( protocol + "api" + params.domain + GetCustomUserUrl,
  ...
  const apiDataAdmin: any[] | any = await getData( protocol + "api" + params.domain +  GetCustomUserUrl,
    ...
    <LayoutAdmin>
      <div className='flex flex-col gap-6 md:gap-16'>
        <Breadcrumb
          ...
          pageName1="Dashboard"
          link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageDashboard`}
        />
...
const SearchUserProfile = ({ params, role }: any) => {
  const onSearchUserProfileServerAction = async (formData: FormData) => {
    ...
    if (value && value.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers?full_name=${value}`)
    }
    redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers`)
    ...
    <div className="bg-white border-4 border-stroke dark:bg-slate-900 dark:border-strokedark p-2 rounded-sm shadow-default">
      <div className="flex justify-between md:px-6 px-2 py-2">
        ...
        <MyButtonCustom
          title={`+ Add Admin`}
          type='add'
          href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/pageAddAdminUser`}
        />
      </div>
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md px-2 text-center text-sm">
            <button>
              {/* <MyButtonCustom
                title='View'
                type='add'
                href={`/${params.domain}/Section-H/pageAdministration/pageLecturers/details/${item.id}`}
              /> */}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="bg-white border-4 border-stroke dark:bg-slate-900 dark:border-strokedark p-2 rounded-sm shadow-default">
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md px-2 text-center text-sm">
            <button>
              {/* <MyButtonCustom
                title='View'
                type='view'
                href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/details/${item.id}`}
              /> */}
            </button>
          </div>
        </div>
      ...
      <MyPagination
        ...
        thisUrl={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents?user__role=student`}
      />
