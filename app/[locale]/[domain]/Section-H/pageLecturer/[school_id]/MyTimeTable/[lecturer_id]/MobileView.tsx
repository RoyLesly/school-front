...
const MobileView = ({ selectedDate, data, yearweek, params }: any) => {
                ...
                [...new Set(yearweek)].map((week: any) => <div key={week} className='bg-blue-800 gap-10 mb-10 p-2 rounded'>
                    ...
                    {Days.map((day: string) => <div key={day} className='bg-blue-100 my-2 p-1 rounded'>
                        ...
                        {data.filter((item: GetTimeSlotInter) => item.timetableday__timetableweek__year_week == week && item.timetableday__day == day).map(
                            (item: GetTimeSlotInter) => <div key={item.id} className='bg-white flex items-center justify-between my-1 py-1 rounded w-full'>
                                <span className='text-sm w-[15%]'>D-{item.start.slice(8, 10)} {item.start.slice(11, 16)}</span>
                                <span className={`${item.status == "CHECKED-IN" ? "bg-blue-300": item.status == "CHECKED-OUT" ? "bg-green-300" : ""} px-1 rounded font-medium italic w-[77%]`}>{item.title}</span>
                                <span className='font-medium italic w-[8%]'><FormModal table='timetable_clock_in_form' type='update' params={params} data={item} icon={<FaEdit />} /></span>
                            </div>
                        )}
                    </div>)
                    }
