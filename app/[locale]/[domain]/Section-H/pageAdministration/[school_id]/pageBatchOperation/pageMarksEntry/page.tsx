...
const page = async ({
  ...
  const apiLecturers = await getData(protocol + "api" + params.domain + GetCustomUserUrl, { ...searchParams, role: "teacher", nopage: true, school__id: params.school_id, is_staff: false })
  const apiAdmins = await getData(protocol + "api" + params.domain + GetCustomUserUrl, { ...searchParams, role: "admin", nopage: true, school__id: params.school_id, is_staff: false })
  ...
  return (
    <div className='flex flex-col h-full'>
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:px-2 md:text-md text-center text-sm">
            <button>
              <MyButtonCustom
                title='View'
                type='view'
                href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry/${item.id}?specialty__academic_year=${sortYears[sortYears.length - 1]}`}
              />
            </button>
          </div>
        </div>
      ...
      <MyPagination
        ...
        nextLink={data.next}
        count={data.count}
        thisUrl={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry?user__role=teacher`}
      />
...
const SearchLecturers = async ({ params }: any) => {
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (name && name.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry?full_name=${name}`)
    }
    redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry`)
