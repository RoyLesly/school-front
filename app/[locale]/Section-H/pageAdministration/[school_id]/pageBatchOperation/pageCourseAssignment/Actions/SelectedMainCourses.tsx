    ...
    const submit = async () => {
        ...
        if (checkCode.length < 1 && checkCredit.length < 1 && checkHours.length < 1 && checkType.length < 1 && pathname) {
            ...
            if (data && data.length > 0 && pn) {
                ...
                .then(res => {
                    ...
                    if (res && res.length > 0 && pn) {
                        ...
                        if (!t.includes("fulfilled")){
                            router.push(`/pageAdministration/${pn[0]}/pageBatchOperation/pageCourseAssignment?error=Operation Successful`);
                        } else {
                            router.push(`/pageAdministration/${pn[0]}/pageBatchOperation/pageCourseAssignment?created=Operation Successful`);
                        }
                        setLoading(false);
                    }
                })
                .catch(err => {
                    if (pn){
                        router.push(`/pageAdministration/${pn[0]}/pageBatchOperation/pageCourseAssignment?specialty_id=${searchParams.get("specialty_id")}&semester=${searchParams.get("semester")}&error=${JSON.stringify(err)}`);
            ...
            const response = await ActionCreate(data[0], SchemaCreateEditCourse, protocol + CourseUrl)
            if (response?.error) {
            }
