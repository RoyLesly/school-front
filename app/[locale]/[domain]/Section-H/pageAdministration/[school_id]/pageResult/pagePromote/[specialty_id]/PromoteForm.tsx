...
const PromoteForm = ({ studentData, params, programs, nextSpecialty }: any) => {
    ...
    useEffect(() => {
        ...
        if (count == 1 && nextSpecialty.count > 0) {
            const nextProf = async () => {
                console.log(45)
                var a = await getData(protocol + "api" + domain + GetUserProfileUrl, {
    ...
    }, [count, nextSpecialty, studentData, studentDataFiltered, studentDataMatricles, domain])
            ...
            await Promise.allSettled(ArrayStudentsToPromote.map((item: SelectedPromoteStudentInter, index: number) => promMine(item)))
                .then(res => {
                    if (res && res.length > 0) {
                        ...
                        console.log(100, t)
                        if (t[0] == "fulfilled") {
                            router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pagePromote?created=PROMOTED SUCCESSFULLY !!!`);
                        }
                        else {
                            router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pagePromote/${params.specialty_id}?error=${t[0]}`);
                        }
            ...
            {studentData ?
                ...
                studentDataFiltered && studentDataFiltered.length > 0 ?
                    ...
                    <div className='dark:text-slate-200 text-black'>
                        ...
                        {programID && session && nextSpecialtyID && ArrayStudentsToPromote && ArrayStudentsToPromote.length > 0 && !submitClicked ? <div className='flex items-center justify-center mb-2 mt-10'>
                            <button
