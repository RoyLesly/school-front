...
import { CustomUserInter, DeptInter } from '@/NoDomain/Utils-H/userControl/userInter';
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          pageName="Classes"
          pageName1="Student"
          link1="/Section-H/Section-H/pageAdministration/pageStudents"
        />
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response?.detail) {
      redirect(`/Section-H/Section-H/pageAdministration/pageLecturers/details?error=Error-${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/Section-H/Section-H/pageAdministration/pageLecturers?updated=Updated-${JSON.stringify(response.full_name).replaceAll(" ", "-")}`)
    } else {
      redirect(`/Section-H/Section-H/pageAdministration/pageLecturers/details/${params.id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }

  const onSubmitActionDelete = async () => {
    'use server'

    const response = await ActionDelete(UserProfileUrl, params.id)

    console.log(95, response)
    if (response?.error) {
      redirect(`/Section-H/Section-H/pageAdministration/pageLecturers/details/${params.id}?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.detail) { 
      redirect(`/Section-H/Section-H/pageAdministration/pageLecturers/details/${params.id}?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.success) {
      const response2 = await ActionDelete(CustomUserUrl, apiData.user.id)
      if (response2?.success) {
        redirect("/Section-H/Section-H/pageAdministration/pageLecturers?deleted=Profile-Deleted-Successfully-!!!")
      }
    }
  }
          ...
          <div className="flex flex-col gap-9">
            ...
            <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
              <div className="border-b border-stroke dark:border-strokedark flex flex-col items-center justify-between md:flex-row px-6.5 py-2">
                ...
                <form action={onSubmitActionDelete} className='flex mb-2'>
