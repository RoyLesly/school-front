...
import { GetPublishUrl, PublishUrl } from '@/NoDomain/Utils-H/appControl/appConfig';
import { SchemaCreateEditPublish } from '@/NoDomain/schemas/schemas';
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          link1={`/Section-H/pageAdministration/${params.school_id}/pageResults/pagePublish`}
        />
...
const PublishConfirm = ({ apiData, params, searchParams }: any) => {
  ...
  const onSubmitSearchAction = async () => {
    ...
    if (response.id == apiData.id) {
      if (value) {
        redirect(`/pageAdministration/${params.school_id}/pageAdmin/pagePortals?success=Portal Openned Successfully`)
      }
      redirect(`/pageAdministration/${params.school_id}/pageAdmin/pagePortals?success=Portal Closed Successfully`)
