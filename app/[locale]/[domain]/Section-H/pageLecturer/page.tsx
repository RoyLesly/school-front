      ...
      if (access) {
        ...
        if (token && token.school) {
          ...
          const callSchool = async () => {
            var res = await getData(protocol + "api" + domain + GetSchoolInfoUrl, { nopage: true, fieldList: [ "id", "campus__id", "campus__name", "region", "campus__region", "town", "school_name" ]})
          ...
          setCount(1);
          if (token.role == "admin") {
            if (token.dept?.includes("Lecturer")) { setLink(`${domain}/pageLecturer`); return }
            // if (token.dept?.includes("Administration")) { setLink(`/pageAdministration`); return }
          } else {
            if (!token.page || token.page.length < 1) {
              if (token.dept?.includes("Lecturer")) { setLink(`${domain}/pageLecturer`); return }
              if (token.dept?.includes("Acounting")) { setLink(`${domain}/pageAcounting`); return }
              if (token.dept?.includes("Administration")) { setLink(`${domain}/pageAdministration`); return }
            } else {
              if (token.page?.includes("MarkUpload")) { setLink("/pageMarkUpload"); return }
              if (token.page?.includes("CourseAssignment")) { setLink(`${domain}/pageCourseAssignment`); return }
              if (token.page?.includes("CourseTimeTable")) { setLink(`${domain}/pageCourseTimeTable`); return }
            }
          }
        }
      } else {
