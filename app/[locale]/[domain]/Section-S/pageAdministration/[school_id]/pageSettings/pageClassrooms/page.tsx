...
const page = async ({
  ...
  const apiData: any = await getData(protocol  + "api" + params.domain + GetClassroomUrl, {...searchParams, school__id: params.school_id, fieldList: [ "id", "level__level", "level__option", "academic_year", "tuition" ]});
  ...
  return (
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex gap-2 justify-between md:px-4 p-2">
        ...
        <MyButtonCustom 
          title="Add Class"
          href={`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/create`}
        />
      ...
      {apiData.results && apiData.results.length > 0 && apiData.results.map((item: GetClassroomInter, key: number) => (
        ...
        >
          ...
          <div className="flex gap-2 items-center justify-end md:gap-6 w-full">
            <MyButtonCustom
              title="View" 
              href={`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/details/${item.id}`}
            />
          </div>
        </div>
      ))}
      ...
      <MyPagination
        ...
        count={apiData.count}
        thisUrl={`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms`}
      />
...
const Search = async ({params}: any) => {
  ...
  const onSearchServerAction = async (formData: FormData) => {
    ...
    if (year) {
      redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms?academic_year=${year}`)
    } 

    redirect(`/${params.domain}/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms`)
