...
import { CustomUserUrl, DepartmentUrl, UserProfileUrl } from '@/NoDomain/Utils-H/userControl/userConfig';
...
const page = async ({
  ...
  const deptData: DeptInter[] | any = await getData(protocol + DepartmentUrl, { ...searchParams, name: "Lecturer" });
  return (
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Lecturers"
          link1={`/Section-H/pageAdministration/${params.school_id}/pageLecturers`}
        />
...
const Create = async ({ params, deptData }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const data = {
      ...
      // school_id: parseInt(params.school_id),
      created_campus: [parseInt(params.school_id)]
    }

    const response = await ActionCreate(data, SchemaCreateCustomUser,protocol + CustomUserUrl, `/pageAdministration/${params.school_id}`)
    ...
    if (response?.id) {
      if (response.id) {
        ...
        const response2 = await ActionCreate(userProfileData, UserProfileLectAdminEditSchema, protocol + UserProfileUrl)
        if (response2.id) {
          redirect(`/Section-H/pageAdministration/${params.school_id}/pageLecturers?created=Successfully Created`, RedirectType.replace)
        }
      }
    }
    // console.log(response)
    if (response?.error) {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response.error)}`)
    }
    if (response?.detail) {
      console.log(80, response.detail)
    }
    if (response) {
      redirect(`/Section-H/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response)}`)
    }
  }
