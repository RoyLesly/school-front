...
import { CustomUserUrl, SecondaryProfileUrl } from '@/Domain/Utils-S/userControl/userConfig';
...
const page = async ({
  ...
  const apiData: UserProfileInter | any = await getData(SecondaryProfileUrl, { id: params.id });
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Student"
          link1="/pageAdministration/pageStudents"
        />
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response?.detail) {
      redirect(`/pageAdministration/pageStudents/details?error=Error-${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/pageAdministration/pageStudents?updated=Updated-${JSON.stringify(response.full_name).replaceAll(" ", "-")}`)
    } else {
      redirect(`/pageAdministration/pageStudents/details/${params.id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(SecondaryProfileUrl, apiData.id)

    console.log(95, response)
    if (response?.error) {
      redirect(`/pageAdministration/pageStudents/details/${params.id}?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.detail) { 
      redirect(`/pageAdministration/pageStudents/details/${params.id}?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.success) {
