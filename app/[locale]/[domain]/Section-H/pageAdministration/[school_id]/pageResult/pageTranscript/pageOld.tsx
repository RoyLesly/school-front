...
const page = async ({
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (matricle && matricle.toString().length > 6) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pageTranscript?user__matricle=${matricle}`)
    } else {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pageTranscript`)
    }
  }
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Back"
          link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult`}
        />
          ...
          apiData.results.map((item: GetUserProfileInter) => (
            <div key={item.id} className="border-stroke border-t dark:border-strokedark even:bg-slate-200 font-semibold grid grid-cols-4 md:grid-cols-11 md:px-6 md:text-lg px-4 py-2">
              ...
              <div className='flex items-center justify-center tracking-widest'>
                <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResult/pageTranscript/${item.id}`} className="bg-bluedark px-4 py-1 rounded text-white">Select</Link>
              </div>
            </div>
          ))
