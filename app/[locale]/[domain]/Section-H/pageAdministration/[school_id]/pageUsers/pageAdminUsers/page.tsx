...
const page = async ({
  ...
  const apiData: CustomUserInter[] | any = await getData(protocol + "api" + params.domain + GetCustomUserUrl, { ...searchParams, school__id: params.school_id, nopage: true, role: "admin", school__campus__id: params.school_id, is_superuser: false });
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Dashboard"
          pageName2="Users"
          link1={`/${params.domain}/${params.domain}/Section-H/pageAdministration`}
          link2={`/${params.domain}/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers`}
        />
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex gap-2 justify-between md:px-3 md:py-3 px-2 py-1 xl:px-7.5">
        ...
        <div className='flex flex-row gap-2'>
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/pageStutsUsers`}>Students</Link></button>
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers`}>Lecturers</Link></button>
        </div>
      ...
      {apiData && apiData.length > 0 && apiData.map((item: CustomUserInter, key: number) => (
        ...
        >
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md">
            <MyButtonCustom
              title='View'
              href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/details/${item.id}`}
              type="view"
            />
          </div>
        </div>
      ...
      <MyPagination
        ...
        count={apiData.count}
        thisUrl={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers`}
      />
...
const SearchUser = ({ params }: any) => {
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (full_name) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/pageAdminUsers?full_name=${full_name}`)
    }
  }
  ...
  return (
    <>
      ...
      <form action={onSearchDrugServerAction} className='flex flex-row gap-1'>
        ...
        <SearchButton />
        <Link className='flex items-center justify-center' href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers`}><FiRefreshCw size={23} /></Link>
      </form>
