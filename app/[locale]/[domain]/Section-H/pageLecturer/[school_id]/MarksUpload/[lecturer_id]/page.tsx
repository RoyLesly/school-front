...
export const metadata: Metadata = {
  title: "Upload",
  description: "This is Upload Page",
};
    ...
    >
      ...
      <td>
        <span className="bg-blue-300 border flex gap-2 h-9 items-center justify-center rounded-full w-9">
          <Link href={`/${params.domain}/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${item.id}`}><FaRightLong /></Link>
        </span>
      </td>
    </tr>
  );
...
const SearchLecturerCourses = async ({ params }: any) => {
  ...
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (value && value.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}?main_course__course_name=${value}&specialty__academic_year=${year}&specialty__level__id=${level}`)
    }
    if (year || level) {
