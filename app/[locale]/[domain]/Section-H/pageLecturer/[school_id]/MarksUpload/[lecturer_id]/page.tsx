...
const page = async ({
  ...
  const coursesData: any = await getData(protocol + "api" + params.domain + GetCourseUrl, { ...searchParams, nopage: true, specialty__school__campus__id: params.school_id, assigned_to__id: params.lecturer_id, fieldList: [
    "id", 
  ...
  return (
    <LayoutLecturer>
      <>
        ...
        {coursesData && coursesData.length > 0 && <List apiData={coursesData} params={params} />}
      ...
      {apiData && apiData.length > 0 && apiData.map((item: GetCourseInter, key: number) => (
        ...
        >
          ...
          <div className="col-span-3 flex items-center">
            ...
            <Link href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${item.id}`} className="flex md:hidden">
              {item.main_course__course_name}
            </Link>
          </div>
          <div className="col-span-3 hidden items-center md:flex">
          ...
          <Link href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${item.id}`} className="col-span-2 flex flex-col gap-0 items-center md:col-span-1 md:flex-row md:justify-between text-sm text-wrap">
          ...
          <div className="gap-2 hidden items-center justify-end md:flex md:gap-6 w-full">
            <MyButtonCustom
              title="Upload"
              href={`/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}/details/${item.id}`}
            />
          </div>
        </div>
      ))}
...
const SearchLecturerCourses = async ({params}: any) => {
  const acadYear = await getData(protocol + "api" + params.domain + AcademicYearUrl, {});
  ...
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (course_name && course_name.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}?main_course__course_name=${course_name}&specialty__academic_year=${year}&specialty__level__id=${level}`)
    } 
    if (year || level) {
      redirect(`/${params.domain}/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}?specialty__academic_year=${year}&specialty__level__id=${level}`)
    }
    redirect(`/${params.domain}/Section-H/pageLecturer/${params.school_id}/MarksUpload/${params.lecturer_id}`)
  ...
  return (
    <>
      ...
      <form action={onSearchDrugServerAction} className='flex flex-row gap-2 text-black'>
        <input name='course_name' placeholder='Search Courses ...' className='border-2 border-slate-700 flex md:w-60 px-2 py-1 rounded text-black w-full' />
