...
const page = async ({
  ...
  const apiData: CustomUserInter[] | any = await getData(protocol + "api" + params.domain + GetCustomUserUrl, { ...searchParams, nopage: true, role: "teacher", school__id: params.school_id, is_superuser: false });
console.log(apiData, 25)
  return (
    <LayoutAdmin>
      <>
        <Breadcrumb
          pageName="Lecturers - Users"
          pageName1="Dashboard"
          pageName2="Users"
          link1={`/${params.domain}/Section-H/pageAdministration`}
          link2={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers`}
        />
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex gap-2 justify-between md:px-3 md:py-3 px-2 py-1 xl:px-7.5">
        ...
        <div className='flex flex-row gap-2'>
          ...
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers/pageAdminUsers`}>Admins</Link></button>
        </div>
...
const SearchUser = ({ params }: any) => {
  const onSearchUserServerAction = async (formData: FormData) => {
    'use server'
    ...
    if (full_name) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUsers?full_name=${full_name}`)
    }
  }
  ...
  return (
    <>
      ...
      <form action={onSearchUserServerAction} className='flex flex-row gap-1'>
        ...
        <button type="submit" className='flex items-center justify-center'><FiRefreshCw size={23} /></button>
      </form>
