...
import { GetPublishUrl, PublishUrl } from '@/Domain/Utils-H/appControl/appConfig';
...
const page = async ({
  ...
  params: { school_id: string, publish_id: string, domain: string };
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {

  const apiData = await getData(protocol + "api" + params.domain + GetPublishUrl, {
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          link1={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageResults/pagePublish`}
        />
...
const PublishConfirm = ({ apiData, params, searchParams }: any) => {
  ...
  const onSubmitSearchAction = async () => {
    ...
    const response = await ActionEdit(data, data.id.toString(), SchemaCreateEditPublish, protocol + "api" + params.domain + PublishUrl)

    if (response.id == apiData.id) {
      if (value) {
        redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePortals?success=Portal Openned Successfully`)
      }
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageUtilities/pagePortals?success=Portal Closed Successfully`)
