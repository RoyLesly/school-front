...
import { DayProgramUrl, TimeTableUrl } from '@/Domain/Utils-H/timeControl/timeConfig';
...
const CreateTimeTable = async ({ params, searchParams, courses }: any) => {
    ...
    const SubmitCreate = async (d: FormData) => {
        ...
        const response = await ActionCreate(data_for_time_table, SchemaCreateEditTimeTable, protocol + "api" + params.domain + TimeTableUrl, `/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable`)
        ...
        if (response.id){
            for (let index = 0; index < Days.length; index++) {
                ...
                var res = await ActionCreate(data_for_day_program, SchemaCreateEditDayProgram, protocol + "api" + params.domain + DayProgramUrl, `/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable`)                
                if (res && res.id && index == Days.length - 1){
                    console.log(101, res);
                    redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable?success=Operation Successful` )
                } 
            }
        } else {
            redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageTimeTable/${params.specialty_id}/create?year=${searchParams.year}&month=${searchParams.month}&error="Error !!!`);
        }
    }
