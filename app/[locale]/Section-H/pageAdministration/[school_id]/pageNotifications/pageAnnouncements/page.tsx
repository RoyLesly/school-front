...
const page = async ({
  ...
  const apiNotification: any = await getData(
    protocol + GetNotificationUrl, { ...searchParams, nopage: true },
  );
...
const Announcements = ({ apiNotification, params }: any) => {
  ...
  return <div className='bg-white flex flex-col gap-3 p-2 rounded'>
    <div className='flex gap-4 items-center justify-end'>
      <Link href={`/Section-H/pageAdministration/${params.school_id}/pageNotifications/pageAnnouncements/create`} className='bg-green-500 font-semibold px-6 py-2 rounded text-black'><span className='font-bold'>+</span> Post Message</Link>
    </div>
    ...
    <div>
      ...
      { apiNotification.length > 0 ? apiNotification.map((item: GetNotificationInter, index: number) => <div key={item.id} className='font-semibold grid grid-cols-11 odd:bg-blue-100 p-2 rounded text-black'>
        ...
        <div className='col-span-1 flex gap-2 items-center justify-center'>
          ...
          <Link href={`/Section-H/pageAdministration/${params.school_id}/pageNotifications/pageAnnouncements/${item.id}`} className='bg-green-500 px-4 py-1 rounded'>View</Link>
        </div>
      </div>)
      : 
