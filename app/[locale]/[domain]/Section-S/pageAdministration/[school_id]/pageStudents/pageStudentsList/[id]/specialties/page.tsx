...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response?.id) {
      ...
      redirect(`/pageAdministration/pageLecturers/details/${params.id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await ActionDelete(SecondaryProfileUrl, params.id)

    console.log(95, response)
    if (response?.error) {
      redirect(`/pageAdministration/pageLecturers/details/${params.id}?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.detail) { 
      redirect(`/pageAdministration/pageLecturers/details/${params.id}?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.success) {
