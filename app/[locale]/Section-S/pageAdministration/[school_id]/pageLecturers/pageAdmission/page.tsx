...
import { SchemaCreateCustomUser } from '@/NoDomain/Utils-S/schemas/schemas-user';
import { ActionCreate } from '@/serverActions/actionGeneral';
import { CustomUserUrl, DepartmentUrl } from '@/NoDomain/Utils-S/userControl/userConfig';
...
const page = async ({
  ...
  const deptData: GetDepartmentInter[] | any = await getData(protocol + DepartmentUrl, { ...searchParams, nopage: true, name: "Lecturer" });
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Lecturers"
          link1={`/Section-S/pageAdministration/${params.school_id}/pageLeturers`}
        />
        ...
        {deptData && deptData.count ? <Create deptData={deptData.results[0]} params={params} /> 
        : 
        <div className="flex flex-col font-semibold gap-10 items-center justify-center pb-32 pt-40 tracking-widest">
          <div className='italic text-2xl'>No Lecturer Department</div>
          <Link href={`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageDepartment/create`} className='bg-bluedark px-6 py-1 rounded text-lg text-white'>New Lecturer Department</Link>
        </div>
        }
...
const Create = async ({ params, deptData }: any) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const data = {
      ...
      prefix: un ? un.toString().toLowerCase() : "",
      role: "teacher",
      dept: [parseInt(dept_id)],
      school: [parseInt(params.school_id)],
    }

    const response = await ActionCreate(data, SchemaCreateCustomUser, protocol + CustomUserUrl, `/Section-S/pageAdministration/${params.school_id}`)

    if (response?.error) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response.error)}`)
    }
    if (response?.id) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageLecturers?success=Lecturer Created`)
    }
    if (response?.detail) {
      console.log(80, response.detail)
    }
    if (response) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageLecturers/pageAdmission?error=${JSON.stringify(response)}`)
    }
  }
              ...
              <form className="flex flex-col gap-4 md:gap-10 p-6.5" action={onSubmitServerAction}>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:gap-10 w-full">
                  ...
                  <div className="flex flex-col w-full">
                    ...
                    <select name="sex" className='border-[1.5px] px-4 py-2 rounded'>
                      <option value={""}>-------------</option>
