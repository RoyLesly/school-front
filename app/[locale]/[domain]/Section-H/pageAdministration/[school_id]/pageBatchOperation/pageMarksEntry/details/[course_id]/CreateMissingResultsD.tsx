  ...
  const createResults = async () => {
    ...
    if (filB && filB.length > 0) {
      ...
      await Promise.allSettled(filB.map((item: GetUserProfileInter, index: number) => {
        ...
        return (
          ActionCreate(data, SchemaCreateEditResult, protocol + "api" + params.domain + ResultUrl)
        )
      }))
        .then(res => {
          ...
          if (res && res.length > 0) {
            ...
            else { router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageMarksEntry?error=Error !!!`); }
          }
        })
        .catch(err => {
          console.log(98, err)
