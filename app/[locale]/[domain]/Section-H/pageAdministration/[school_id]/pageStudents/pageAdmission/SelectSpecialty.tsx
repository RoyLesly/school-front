'use client';
import { GetDomainUrl, GetLevelUrl, GetSpecialtyUrl } from '@/Domain/Utils-H/appControl/appConfig';
...
const SelectSpecialty = ({ school_id }: any) => {
    ...
    useEffect(() => {
        if (count == 0) {
            const getLevels = async () => {
                var res = await getData(protocol + "api" + dom + GetLevelUrl, {})
                if (res && res.results) { setLevelData(res.results) }
            }
            const getDomains = async () => {
                var res = await getData(protocol + "api" + dom + GetDomainUrl, {})
                if (res && res.results) { setDomainData(res.results) }
            }
            const getPrograms = async () => {
                var res = await getData(protocol + "api" + dom + ProgramUrl, {})
        ...
        if (count == 1) {
            if (domainDataID && yearDataID && levelDataID) {
                const getSpecialties = async () => {
                    var res = await getData(protocol + "api" + dom + GetSpecialtyUrl, { 
    ...
    }, [count, school_id, domainDataID, yearDataID, levelDataID, dom])
