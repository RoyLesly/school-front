...
const page = async ({
  ...
  const coursesData: any = await getData(protocol + "api" + params.domain + GetCourseUrl, { ...searchParams, nopage: true, assigned_to__id: params.lecturer_id, specialty__school__campus__id: params.school_id, fieldList: [
    "id", 
        ...
        {apiData && apiData.length > 0 ? apiData.map((item: GetCourseInter, key: number) => (
          ...
          >
            ...
            <div className="col-span-3 flex items-center">
              ...
              <Link href={`/Section-H/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}/details/${item.id}`} className="flex md:hidden underline">
                {item.main_course__course_name}
              </Link>
            </div>
            <div className="col-span-3 hidden items-center md:flex">
            ...
            <div className="gap-2 hidden items-center justify-end md:flex md:gap-6 w-full">
              <MyButtonCustom
                title="View"
                href={`/Section-H/pageLecturer/${params.school_id}/MyCourses/${params.lecturer_id}/details/${item.id}`}
              />
            </div>
          </div>
        ))
