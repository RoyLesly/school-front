...
import { CustomUserUrl, GetCustomUserUrl } from '@/NoDomain/Utils-S/userControl/userConfig';
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params, searchParams }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    if (response?.detail) { }
    if (response?.id) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageUsers?updated=Updated-${JSON.stringify(response.full_name).replaceAll(" ", "-")}`)
    } else {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageUsers/details/${params.user_id}?error=Error-${JSON.stringify(response).replaceAll(" ", "-")}`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await DeleteAction(CustomUserUrl, params.user_id)
