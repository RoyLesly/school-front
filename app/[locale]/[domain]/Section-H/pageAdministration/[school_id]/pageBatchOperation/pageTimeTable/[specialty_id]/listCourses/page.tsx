...
const page = async ({
  ...
  const apiDayProgram: any = await getData(protocol + "api" + params.domain + GetDayProgramUrl, {
    ...searchParams, timetable__specialty__id: params.specialty_id, fieldList: [
      ...
      "period_0812__main_course__course_name", "period_1317__main_course__course_name", "period_1721__main_course__course_name",
    ]
  });
  ...
  return (
    <LayoutAdmin>
      <>
        ...
        {apiSpecialty && apiSpecialty.count && <List params={params} school_id={params.school_id} data={apiDayProgram} specialty={apiSpecialty} specialty_id={params.specialty_id} />}
...
const List = async ({ data, specialty, school_id, specialty_id, params }: any) => {
  ...
  return <>
    <div className='flex-col flex-grow hidden md:flex'>
      ...
      {specialty && specialty.count ?
        <div className='flex flex-row font-semibold gap-2 items-center justify-between md:text-2xl py-2'>
          <span><Link href={`/${params.domain}/Section-H/pageAdministration/${school_id}/pageBatchOperation/pageTimeTable/${specialty_id}/listCourses`} className='bg-slate-300 px-6 py-1 rounded-sm text-black'>Show Courses</Link></span>
          <span><Link href={`/${params.domain}/Section-H/pageAdministration/${school_id}/pageBatchOperation/pageTimeTable/${specialty_id}/listLecturers`} className='bg-slate-200 px-6 py-1 rounded-sm text-slate-400'>Show Lecturers</Link></span>
          <span className=''><Link className='bg-green-500 flex gap-2 items-center justify-center px-10 py-1 rounded text-white' href={`/${params.domain}/Section-H/pageAdministration/${school_id}/pageBatchOperation/pageTimeTable/${specialty_id}/selectdate`}><FaPlus /><>Create</></Link></span>
        </div> : <></>}
              ...
              <div key={item.id} className='dark:odd:bg-slate-700 odd:bg-blue-200'>
                {item.timetable__year_week == p && <div key={item.id}>
                  <div className="dark:border-strokedark dark:text-white grid grid-cols-4 md:grid-cols-11 px-2 text-black tracking-wider">
                    ...
                    <div className="col-span-3 items-center justify-center md:flex"><span className="font-medium">{item.period_0812__main_course__course_name}</span></div>
                    <div className="col-span-3 items-center justify-center md:flex"><span className="font-medium">{item.period_1317__main_course__course_name}</span></div>
                    <div className="col-span-3 items-center justify-center md:flex"><span className="font-medium">{item.period_1721__main_course__course_name}</span></div>
                    <div className="bg-bluedash flex items-center justify-center my-1 rounded text-center text-white">
                      <Link href={`/${params.domain}/Section-H/pageAdministration/${school_id}/pageBatchOperation/pageTimeTable/${specialty_id}/edit/${item.id}`} className="font-medium">Edit</Link>
                    </div>
                  </div>
                </div>
                }
