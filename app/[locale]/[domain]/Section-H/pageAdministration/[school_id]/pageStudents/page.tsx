...
import { LevelInter } from '@/Domain/Utils-H/appControl/appInter'
        ...
        {apiData && apiData != "ECONNREFUSED" && <>
          {apiData.count ? <List apiData={apiData} params={params} searchParams={searchParams} /> 
            ...
            <div className="bg-white flex flex-col font-semibold gap-10 items-center justify-center pb-60 pt-40 rounded tracking-widest">
              ...
              <Link href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/pageAdmission`} className="bg-bluedark px-6 py-2 rounded text-[20px] text-white">New Student</Link>
            </div>
            }
        </>}  
  ...
  return (
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
        ...
        >
          ...
          <div className="flex gap-2 items-center justify-center md:gap-2 md:text-md px-2 text-center">
            <MyButtonCustom
              title='View'
              type='edit'
              href={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents/details/${item.id}`}
              />
          </div>
        </div>
      ))}
      ...
      <MyPagination
        ...
        count={apiData.count}
        thisUrl={`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents`}
      />
...
const SearchUserProfile = async ({params}: any) => {
  ...
  const onSearchDrugServerAction = async (formData: FormData) => {
    ...
    if (field && field == "specialty" && value && value.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents?specialty__main_specialty__specialty_name=${value}&specialty__academic_year=${year}&specialty__level__id=${level}`)
    } 
    if (field && value && value.toString().length > 1) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents?user__${field}=${value}&specialty__academic_year=${year}&specialty__level__id=${level}`)
    } 
    if (year || level) {
      redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents?specialty__academic_year=${year}&specialty__level__id=${level}`)
    }

    redirect(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageStudents`)
