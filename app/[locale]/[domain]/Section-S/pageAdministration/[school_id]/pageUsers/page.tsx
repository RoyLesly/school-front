...
const page = async ({
  ...
  const apiData: CustomUserInter[] | any = await getData(protocol + GetCustomUserUrl, { ...searchParams, nopage: true, role: "teacher", school__id: params.school_id, is_staff: false });
  ...
  return (
    <LayoutAdmin>
      <>
        ...
        {apiData && apiData.length > 0 && <List apiData={apiData} searchParams={searchParams} school_id={params.school_id} />}
    ...
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark rounded-sm shadow-default">
      <div className="flex gap-2 justify-between md:px-3 md:py-3 px-2 py-1 xl:px-7.5">
        ...
        <div className='flex flex-row gap-2'>
          <button className='bg-bluelight font-medium px-6 py-1 rounded text-black text-lg'><Link href={`/Section-S/pageAdministration/${school_id}/pageUsers/pageStutsUsers`}>Students</Link></button>
      ...
      {apiData && apiData.length > 0 && apiData.map((item: GetCustomUserInter, key: number) => (
        <div
