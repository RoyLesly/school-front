import { ClassroomUrl, GetClassroomUrl, GetLevelUrl } from '@/NoDomain/Utils-S/appControl/appConfig';
...
const page = async ({
  ...
  const apiData: any = await getData(protocol + GetClassroomUrl, {
    ...
    <LayoutAdmin>
      <>
        <Breadcrumb
          ...
          pageName1="Back"
          link1={`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms`}
        />
...
const EditDelete: FC<EditDeleteProps> = async ({ apiData, params }) => {
  ...
  const onSubmitServerAction = async (formData: FormData) => {
    ...
    const response = await ActionEdit(data, params.classroom_id, SchemaCreateEditClassroom, ClassroomUrl)

    if (response.error) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/details/${params.specialty_id}?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.errors) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/details/${params.specialty_id}?error=${JSON.stringify(response.errors).replaceAll(" ", "-")}`)
    }
    if (response?.detail) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms/details/${params.specialty_id}?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.id) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageClassrooms?updated=Successfully-Updated-${JSON.stringify(response.level.level).replaceAll(" ", "-")}`)
    }
  }
  ...
  const onSubmitDeleteAction = async () => {
    ...
    const response = await DeleteAction(ClassroomUrl, params.specialty_id)

    if (response?.error) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageSpecialties/details/${params.specialty_id}?error=${JSON.stringify(response.error).replaceAll(" ", "-")}`)
    }
    if (response?.detail) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageSpecialties/details/${params.specialty_id}?error=${JSON.stringify(response.detail).replaceAll(" ", "-")}`)
    }
    if (response?.success) {
      redirect(`/Section-S/pageAdministration/${params.school_id}/pageSettings/pageSpecialties?deleted=Successfuly-Deleted-!!!`)
    }
  }
